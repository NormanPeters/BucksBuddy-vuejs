...
import EditableInputField from '@/components/atoms/EditableInputField.vue';
...
const fetchExpenditures = async (journeyId: number) => {
  ...
  try {
    const [expendituresResponse, homeCurrencyResponse, vacCurrencyResponse] = await Promise.all([
      api.getAllExpenditures(journeyId),
      api.getHomeCurrency(journeyId),
      api.getVacCurrency(journeyId),
    ]);

    expendituresList.value = expendituresResponse.data.map((expenditure: Expenditure) => ({
      ...expenditure,
      isEditing: false,
    }));
    ...
    console.error('Error fetching expenditures or currencies:', error);
  }
};
    ...
    <table class="table table-hover align-text-bottom">
      ...
      >
        ...
        <td class="col-3 align-middle" v-else>
          <EditableInputField v-model="item.name" @enter="saveExpenditure(item.id, item)" />
        </td>
        ...
        <td class="col-2 align-middle" v-else>
          <EditableInputField v-model="item.amount" type="number" @enter="saveExpenditure(item.id, item)" />
        </td>
        ...
        <td class="col-2 align-middle" v-else>
          <EditableInputField :value="amountInHomeCurrency(item.amount).toFixed(2)" type="number" disabled />
        </td>
        ...
        <td class="col-3 align-middle" v-else>
          <EditableInputField v-model="item.date" type="date" @enter="saveExpenditure(item.id, item)" />
        </td>
